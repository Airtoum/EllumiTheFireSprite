...
public class OLDDynamicCharacter : Character
{
    ...
    protected virtual void AIInput()
    {
        ...
        */
        ...
        if (AItimeTillRecalculatePath < 0) {
            (float, List<(Vector2, AIMoves)>) result = ExplorePlatform();
            float fitness = result.Item1;
            AItimeTillRecalculatePath = 2f / (fitness * fitness * fitness + 1); // 0 -> 2, 1 -> 1, higher -> 0
            AIcurrentPath = result.Item2;
        } else {
            for(int i = 0; i < AIcurrentPath.Count - 1; i++) {
                Vector2 start = AIcurrentPath[i].Item1;
                Vector2 end = AIcurrentPath[i + 1].Item1;
                Debug.DrawLine(start, end, Color.white);
            }
            if (AIcurrentPath.Count <= 0) {
                inputFlags = 0;
            } else {
                AICurrentMove = AIcurrentPath[0].Item2;
                for (int i = 0; i < AIcurrentPath.Count; i++) {
                    if (Vector2.Distance(transform.position, AIcurrentPath[i].Item1) <= AIPathFollowNodeDistance) {
                        // we might skip ahead on accident
                        AIcurrentPath.RemoveRange(0, i);
                    }
                }
            }
            AItimeTillRecalculatePath -= Time.fixedDeltaTime;
        }
    }

    protected (float, List<(Vector2, AIMoves)>) ExplorePlatform()
    {
        debug_AINodeCount = 0;
        ...
        AIMoveSetInput(selected_input, lateral_choice, backward_choice);
        ...
        // nasty move cleaning; I should have just used left and right directly instead of "lateral"
        List<(Vector2, AIMoves)> cleaned_moves = new List<(Vector2, AIMoves)>();
        for(int i = 0; i < chosen_path.Count; i++) {
            AIMoves move = chosen_path[i].Item2;
            if (move == AIMoves.Lateral) {
                move = (lateral_choice == INPUT_LEFT) ? AIMoves.Left : AIMoves.Right;
            }
            if (move == AIMoves.LateralUp || move == AIMoves.JumpSideways) {
                move = (lateral_choice == INPUT_LEFT) ? AIMoves.JumpLeft : AIMoves.JumpRight;
            }
            if (move == AIMoves.JumpBackwards) {
                move = (backward_choice == INPUT_LEFT) ? AIMoves.JumpLeft : AIMoves.JumpRight;
            }
            cleaned_moves.Add((chosen_path[i].Item1, move));
        }
        ...
        return (best_score, cleaned_moves);
    }

    private void AIMoveSetInput(AIMoves selected_move, int lateral_choice, int backward_choice)
    {
